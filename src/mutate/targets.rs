...
use super::fields::{
    ...
    EROFS_DEVT_SLOT_SIZE, EROFS_FEATURE_INCOMPAT_48BIT, EROFS_FEATURE_INCOMPAT_DEVICE_TABLE,
    EROFS_FEATURE_INCOMPAT_FRAGMENTS, EROFS_INODE_CHUNK_BASED, EROFS_INODE_COMPRESSED_COMPACT,
    EROFS_INODE_COMPRESSED_FULL, EROFS_INODE_SLOT_SIZE, INODE_FIELDS, SUPERBLOCK_FIELDS,
    Z_EROFS_ADVISE_BIG_PCLUSTER_1, Z_EROFS_ADVISE_UNSUPPORTED_BIT,
    Z_EROFS_CLUSTERBITS_RESERVED_BIT, Z_EROFS_FRAGMENT_INODE_MASK, Z_EROFS_MAP_HEADER_SIZE,
    inode_field_location,
};
use crate::checksum::fix_checksum;
use crate::cli::MutateArgs;
use crate::dirent::locate_dirents_in_image;
use crate::image::{EROFS_SUPER_OFFSET, FieldWidth, Image, write_image};
use crate::inode::{inode_data_offset, is_directory_inode, is_extended_inode, locate_inodes};
use anyhow::Result;
use std::path::Path;

pub(super) fn mutate_superblock(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
...
pub(super) fn mutate_inodes(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
    let seed = seed_name(&args.input);
...
pub(super) fn mutate_dirents(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
    let seed = seed_name(&args.input);
    let sb = image.superblock()?;
    let inodes = locate_inodes(image, &sb)?;
...
pub(super) fn mutate_chunks(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
    let seed = seed_name(&args.input);
    let sb = image.superblock()?;
    let inodes = locate_inodes(image, &sb)?;
    let mut entries = Vec::new();
...
pub(super) fn mutate_compressions(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
    let seed = seed_name(&args.input);
    let sb = image.superblock()?;
    let inodes = locate_inodes(image, &sb)?;
    let mut entries = Vec::new();
...
pub(super) fn mutate_fragments(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
    let seed = seed_name(&args.input);
    let sb = image.superblock()?;
    let inodes = locate_inodes(image, &sb)?;
    let mut entries = Vec::new();
...
pub(super) fn mutate_devices(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
    let seed = seed_name(&args.input);
    let sb = image.superblock()?;
    let mut entries = Vec::new();
...
pub(super) fn mutate_xattrs(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
    let seed = seed_name(&args.input);
    let sb = image.superblock()?;
    let inodes = locate_inodes(image, &sb)?;
    let mut entries = Vec::new();
...
pub(super) fn mutate_cross_fields(image: &Image, args: &MutateArgs) -> Result<Vec<MutatedEntry>> {
    let seed = seed_name(&args.input);
    let sb = image.superblock()?;
    let inodes = locate_inodes(image, &sb)?;
    let dirents = locate_dirents_in_image(image, &sb, &inodes)?;
